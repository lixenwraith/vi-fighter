...
| `k3s/session.sh` | Repeatable manual create/list/delete/status/blockers/drain path. `blockers` is the annotated emptiness assertion every update helper runs; `drain [--force]` is what empties the fleet for them. It creates the Job first, owns the Service by the returned Job UID, selects a free fleet port when omitted, and cleans a partial create. |
