...
| `docker/Dockerfile.logwisp` | Multi-stage LogWisp sidecar build. Use the LogWisp repository as its build context; the final layer is one static non-root binary. |
...
| `k3s/50-logwisp.yaml` | The log sidecar's configuration. Not what the deployment runs: its file source cannot carry vi-fighter's envelope intact, so it is what the H9 in-pod experiment tests against. |
