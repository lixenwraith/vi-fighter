...
| `docker/Dockerfile.ws-bridge` | The browser bridge sidecar: one verified static WebSocket-to-loopback-TCP binary in a `scratch` layer, non-root, no shell. The release checksum is a required build argument, so an unpinned build fails. |
...
| `k3s/20-networkpolicy.yaml` | Default deny in both directions; the game port from anywhere, the operator ports from monitoring only, no egress. The private bridge port gets no rule: the default deny is what keeps every pod off it, and the node reaches it the way it already reaches `/health`. |
| `k3s/30-session.yaml` | The per-session template: one Restricted Job writing its commissioned JSONL through the shared log PVC, reading its scenario through the read-only wad PVC, and one owned NodePort Service. `${SCENARIO}` and `${LOG_LEVEL}` are the allocator's per-session choices, and `${WS_BRIDGE_IMAGE}` adds the browser bridge sidecar. |
...
| `k3s/render-session.sh` | Renders the template with lifetime, `SCENARIO` and `LOG_LEVEL` overrides. `WS_BRIDGE_IMAGE=<image>` adds the bridge sidecar and omitting it renders the native-clients-only session. `JOB_UID=<uid>` retains the Service owner reference. |
