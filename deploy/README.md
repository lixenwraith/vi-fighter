...
| `k3s/30-session.yaml` | The per-session template: one Restricted Job writing its commissioned JSONL through the shared log PVC, reading its scenario through the read-only wad PVC, and one owned NodePort Service. `${SCENARIO}` and `${LOG_LEVEL}` are the allocator's per-session choices. |
