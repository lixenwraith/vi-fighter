...
func drawDiagMode() {
	...
	dimFg := color.RGB{120, 120, 120}

	drawText(1, startY, "H:Hex input  ↑↓:R±1  ←→:G±1  PgUp/Dn:B±1", color.RGB{100, 100, 100}, bg)
	startY += 2

	// Current color display
	drawBox(0, startY, 78, 7, " Color Analysis ", color.RGB{80, 80, 80}, bg)
	...
	if redmeanDelta < naiveDelta {
		drawText(63, startY+4, "◀ closer", color.RGB{100, 255, 100}, bg)
	} else if naiveDelta < redmeanDelta {
		drawText(63, startY+5, "◀ closer", color.RGB{100, 255, 100}, bg)
	} else {
		drawText(63, startY+4, "= equal", color.RGB{200, 200, 100}, bg)
	}

	startY += 8

	// Quantization trace
	drawBox(0, startY, 78, 8, " Redmean LUT Trace ", color.RGB{80, 80, 80}, bg)
	r6 := state.diagInputRGB.R >> 2
	g6 := state.diagInputRGB.G >> 2
	b6 := state.diagInputRGB.B >> 2
	lutIdx := int(r6)<<12 | int(g6)<<6 | int(b6)

	drawText(2, startY+1, fmt.Sprintf("1. Input RGB:    (%3d, %3d, %3d)", state.diagInputRGB.R, state.diagInputRGB.G, state.diagInputRGB.B), color.RGB{200, 200, 100}, bg)
	drawText(2, startY+2, fmt.Sprintf("2. Quantize >>2: (%3d, %3d, %3d)  [6-bit per channel]", r6, g6, b6), color.RGB{200, 200, 100}, bg)
	drawText(2, startY+3, fmt.Sprintf("3. LUT index:    %d<<12 | %d<<6 | %d = %d", r6, g6, b6, lutIdx), color.RGB{200, 200, 100}, bg)
	drawText(2, startY+4, fmt.Sprintf("4. LUT[%d] → palette index %d", lutIdx, info.Redmean256), color.RGB{200, 200, 100}, bg)
	drawText(2, startY+5, fmt.Sprintf("5. Palette[%d] → RGB (%3d, %3d, %3d)", info.Redmean256, info.Redmean256Bg.R, info.Redmean256Bg.G, info.Redmean256Bg.B), color.RGB{200, 200, 100}, bg)
	startY += 9

	// Naive cube trace
	drawBox(0, startY, 78, 5, " Naive Cube Trace ", color.RGB{80, 80, 80}, bg)
	ri := snapToCube(int(state.diagInputRGB.R))
	gi := snapToCube(int(state.diagInputRGB.G))
	bi := snapToCube(int(state.diagInputRGB.B))
	drawText(2, startY+1, fmt.Sprintf("1. Snap R=%3d → cube[%d]=%3d", state.diagInputRGB.R, ri, cubeValues[ri]), color.RGB{200, 200, 100}, bg)
	drawText(2, startY+2, fmt.Sprintf("2. Snap G=%3d → cube[%d]=%3d", state.diagInputRGB.G, gi, cubeValues[gi]), color.RGB{200, 200, 100}, bg)
	drawText(2, startY+3, fmt.Sprintf("3. Snap B=%3d → cube[%d]=%3d   idx = 16 + %d*36 + %d*6 + %d = %d",
		state.diagInputRGB.B, bi, cubeValues[bi], ri, gi, bi, info.Naive256), color.RGB{200, 200, 100}, bg)

	// Hex input overlay
	if state.hexInputActive && state.hexInputTarget == 1 {
		drawBox(20, 10, 30, 5, " Hex Input ", color.RGB{255, 255, 0}, color.RGB{40, 40, 60})
...
func deltaColor(delta int) color.RGB {
	if delta < 15 {
		return color.RGB{100, 255, 100} // Green - good
	} else if delta < 40 {
		return color.RGB{255, 255, 100} // Yellow - ok
	}
