...
func drawPaletteMode() {
	...
	// Keys help
	drawText(1, startY, "↑↓:Select PgUp/Dn:Jump Home/End:First/Last", color.RGB{100, 100, 100}, bg)
	startY += 2
	...
	drawText(35, startY, "TC", color.RGB{100, 180, 100}, bg)
	drawText(40, startY, "256", color.RGB{180, 180, 100}, bg)
	...
	// Separator line
	for x := 1; x < 80; x++ {
		drawSwatchChar(x, startY, '─', color.RGB{60, 60, 60}, bg)
	}
	startY++
	...
	for i := 0; i < listHeight && state.paletteScroll+i < len(gamePalette); i++ {
		...
		// Index
		drawText(1, y, fmt.Sprintf("%2d", idx), color.RGB{100, 100, 100}, rowBg)

		// Group
		drawText(5, y, fmt.Sprintf("%-10s", entry.Group), color.RGB{120, 120, 120}, rowBg)
		...
		// RGB values
		drawText(45, y, fmt.Sprintf("(%3d,%3d,%3d)", entry.Color.R, entry.Color.G, entry.Color.B), color.RGB{150, 150, 150}, rowBg)

		// Hex
		drawText(60, y, fmt.Sprintf("#%02X%02X%02X", entry.Color.R, entry.Color.G, entry.Color.B), color.RGB{150, 150, 150}, rowBg)

		// 256 index
		drawText(69, y, fmt.Sprintf("%3d", idx256), color.RGB{130, 130, 130}, rowBg)

		// Delta indicator (small visual if significant difference)
		delta := absDelta(entry.Color, rgb256)
		if delta > 50 {
			drawText(77, y, "!", color.RGB{255, 100, 100}, rowBg)
		} else if delta > 25 {
			drawText(77, y, "~", color.RGB{255, 200, 100}, rowBg)
		}
	}
	...
	if len(gamePalette) > listHeight {
		...
		drawText(state.width-len(scrollInfo)-2, startY-2, scrollInfo, color.RGB{100, 100, 100}, bg)
	}

	// Detailed info for selected color
	infoY := startY + listHeight + 1
	drawBox(0, infoY, 80, 8, " Selected Color Analysis ", color.RGB{80, 80, 80}, bg)
	...
	if state.paletteIdx < len(gamePalette) {
		...
		drawText(2, infoY+5, lutNote, color.RGB{180, 180, 100}, bg)
	}
}
