...
func drawBlendMode() {
	...
	// Keys help
	drawText(1, startY, "S:Src D:Dst O:Op ←→:Alpha(±0.01) Shift:±0.1 B:Bg H:HexInput R:Reset", color.RGB{100, 100, 100}, bg)
	startY += 2
	...
	drawText(31, startY, fmt.Sprintf("(%3d,%3d,%3d)", srcColor.R, srcColor.G, srcColor.B), color.RGB{150, 150, 150}, bg)
	startY++

	// Destination
	drawText(1, startY, fmt.Sprintf("DST: %-18s", gamePalette[state.blendDstIdx].Name), fg, bg)
	drawSwatch(25, startY, 5, dstColor)
	drawText(31, startY, fmt.Sprintf("(%3d,%3d,%3d)", dstColor.R, dstColor.G, dstColor.B), color.RGB{150, 150, 150}, bg)
	startY++

	// Operation
	drawText(1, startY, fmt.Sprintf("OP:  %-12s", op.name), color.RGB{255, 200, 100}, bg)
	drawText(20, startY, fmt.Sprintf("α: %.2f", state.blendAlpha), color.RGB{100, 200, 255}, bg)
	startY++
	...
	// Formula
	drawBox(0, startY, 80, 3, " Formula ", color.RGB{80, 80, 80}, bg)
	drawText(2, startY+1, op.formula, color.RGB{200, 200, 100}, bg)
	startY += 4
	...
	// Results box
	drawBox(0, startY, 80, 10, " Result ", color.RGB{80, 80, 80}, bg)
	startY++

	// TrueColor result
	drawText(2, startY, "TrueColor:", fg, bg)
	drawSwatch(15, startY, 8, result)
	drawText(24, startY, fmt.Sprintf("(%3d,%3d,%3d) #%02X%02X%02X", result.R, result.G, result.B, result.R, result.G, result.B), color.RGB{150, 150, 150}, bg)
	startY++
	...
	drawText(24, startY, fmt.Sprintf("idx=%3d → (%3d,%3d,%3d)", info.Redmean256, info.Redmean256Bg.R, info.Redmean256Bg.G, info.Redmean256Bg.B), color.RGB{150, 150, 150}, bg)
	startY++

	drawText(2, startY, "256 Naive:", fg, bg)
	drawSwatch(15, startY, 8, info.Naive256RGB)
	drawText(24, startY, fmt.Sprintf("idx=%3d → (%3d,%3d,%3d)", info.Naive256, info.Naive256RGB.R, info.Naive256RGB.G, info.Naive256RGB.B), color.RGB{150, 150, 150}, bg)
	startY += 2
	...
	drawText(25, startY, "  256 ", color.RGB{0, 0, 0}, info.Redmean256Bg)
