...
func drawBox(x, y, w, h int, title string, fg, bg color.RGB) {
	...
	if title != "" && len(title)+2 < w-2 {
		tx := x + (w-len(title))/2
		drawText(tx, y, title, color.RGB{255, 255, 0}, bg)
	}
...
func DrawColorInfoCompact(x, y int, c color.RGB, label string) {
	...
	drawText(x+labelLen+4, y, fmt.Sprintf("(%3d,%3d,%3d) #%02X%02X%02X", c.R, c.G, c.B, c.R, c.G, c.B), color.RGB{150, 150, 150}, bg)
}
...
func drawColorInfo(x, y int, info ColorInfo) int {
	...
	drawText(x, line, fmt.Sprintf("Delta (Redmean): R%+4d G%+4d B%+4d", info.DeltaR, info.DeltaG, info.DeltaB), color.RGB{150, 150, 150}, bg)
	line++
