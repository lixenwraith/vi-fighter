...
func TestParagraphMotions(t *testing.T) {
	...
	setCursorPosition(ctx, 0, 2)
	ExecuteMotion(ctx, '}', 1)
	if getCursorY(ctx) != 4 {
		t.Errorf("} motion failed: expected Y=4, got Y=%d", getCursorY(ctx))
	}

	// Test } again
	ExecuteMotion(ctx, '}', 1)
	if getCursorY(ctx) != 7 {
		t.Errorf("} motion failed: expected Y=7, got Y=%d", getCursorY(ctx))
	}
...
func TestParagraphMotionCountHandling(t *testing.T) {
	...
	if getCursorY(ctx) != 4 {
		t.Errorf("2} motion: expected Y=4, got Y=%d", getCursorY(ctx))
	}
	...
	// Third }: 4 -> 6
	if getCursorY(ctx) != 6 {
		t.Errorf("3} motion: expected Y=6, got Y=%d", getCursorY(ctx))
	}
	...
	// Second {: 10 -> 9
	if getCursorY(ctx) != 9 {
		t.Errorf("2{ motion: expected Y=9, got Y=%d", getCursorY(ctx))
	}
	...
	if getCursorY(ctx) != 0 {
		t.Errorf("100{ motion: expected Y=0 (top), got Y=%d", getCursorY(ctx))
	}
	...
	if getCursorY(ctx) < 12 {
		t.Errorf("100} motion: expected Y>=12 (stopped at boundary), got Y=%d", getCursorY(ctx))
	}
}
