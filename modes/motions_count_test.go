...
func TestMotionWithCount(t *testing.T) {
	...
	if getCursorY(ctx) != 0 {
		t.Errorf("H with count: expected Y=0, got Y=%d", getCursorY(ctx))
	}
	...
	if getCursorY(ctx) != 4 {
		t.Errorf("} with count=2: expected Y=4, got Y=%d", getCursorY(ctx))
	}
}
...
func TestMotionCountHandling(t *testing.T) {
	...
	if getCursorX(ctx) != 14 {
		t.Errorf("3w motion: expected X=14 (at 'f' in 'four'), got X=%d", getCursorX(ctx))
	}
	...
	if getCursorX(ctx) != 4 {
		t.Errorf("2b motion: expected X=4 (at 't' in 'two'), got X=%d", getCursorX(ctx))
	}
	...
	if getCursorX(ctx) != 6 {
		t.Errorf("2e motion: expected X=6 (at 'o' in 'two'), got X=%d", getCursorX(ctx))
	}
	...
	// Second W: 4 -> 8 ('t' in "three")
	if getCursorX(ctx) != 8 {
		t.Errorf("2W motion: expected X=8 (at 't' in 'three'), got X=%d", getCursorX(ctx))
	}
	...
	setCursorPosition(ctx, 14, getCursorY(ctx)) // at 'f' in "four"
	ExecuteMotion(ctx, 'B', 2)
	// First B: 14 -> 8 ('t' in "three")
	// Second B: 8 -> 4 ('t' in "two")
	if getCursorX(ctx) != 4 {
...
func TestMotionCountBoundaryHandling(t *testing.T) {
	...
	if getCursorX(ctx) != 8 {
		t.Errorf("100w motion: expected X=8 (at 't' in 'three'), got X=%d", getCursorX(ctx))
	}
	...
	if getCursorX(ctx) != 0 {
		t.Errorf("100b motion: expected X=0 (at 'o' in 'one'), got X=%d", getCursorX(ctx))
	}
	...
	if getCursorX(ctx) != 0 {
		t.Errorf("100h motion: expected X=0 (left edge), got X=%d", getCursorX(ctx))
	}
	...
	if getCursorX(ctx) != ctx.GameWidth-1 {
		t.Errorf("100l motion: expected X=%d (right edge), got X=%d", ctx.GameWidth-1, getCursorX(ctx))
	}

	// Test 100j - should stop at bottom
	setCursorPosition(ctx, getCursorX(ctx), 5)
	ExecuteMotion(ctx, 'j', 100)
	if getCursorY(ctx) != ctx.GameHeight-1 {
		t.Errorf("100j motion: expected Y=%d (bottom), got Y=%d", ctx.GameHeight-1, getCursorY(ctx))
	}
	...
	if getCursorY(ctx) != 0 {
		t.Errorf("100k motion: expected Y=0 (top), got Y=%d", getCursorY(ctx))
	}
	...
	if getCursorX(ctx) != ctx.GameWidth-1 {
		t.Errorf("100 space motion: expected X=%d (right edge), got X=%d", ctx.GameWidth-1, getCursorX(ctx))
	}
}
...
func TestMotionCountMixedContent(t *testing.T) {
	...
	if getCursorX(ctx) != 10 {
		t.Errorf("4w with punctuation: expected X=10 (at 'b' in 'baz'), got X=%d", getCursorX(ctx))
	}
	...
	// Fifth w: 10 -> 13 (',')
	// Sixth w: 13 -> 15 ('q')
	if getCursorX(ctx) != 15 {
		t.Errorf("6w with punctuation: expected X=15 (at 'q' in 'qux'), got X=%d", getCursorX(ctx))
	}
	...
	if getCursorX(ctx) != 15 {
		t.Errorf("10w with punctuation: expected X=15 (stopped at 'q'), got X=%d", getCursorX(ctx))
	}
}
...
func TestMotionCountZeroDefaultsToOne(t *testing.T) {
	...
	if getCursorX(ctx) != 4 {
		t.Errorf("0w motion (should default to 1): expected X=4, got X=%d", getCursorX(ctx))
	}
	...
	if getCursorX(ctx) != 4 {
		t.Errorf("0h motion (should default to 1): expected X=4, got X=%d", getCursorX(ctx))
	}
}
...
func TestMotionEdgeCases(t *testing.T) {
	...
	if getCursorX(ctx) != 10 {
		t.Errorf("w on empty line: should stay at X=10, got X=%d", getCursorX(ctx))
	}

	ExecuteMotion(ctx, 'e', 1)
	if getCursorX(ctx) != 10 {
		t.Errorf("e on empty line: should stay at X=10, got X=%d", getCursorX(ctx))
	}

	ExecuteMotion(ctx, 'b', 1)
	if getCursorX(ctx) != 10 {
		t.Errorf("b on empty line: should stay at X=10, got X=%d", getCursorX(ctx))
	}
