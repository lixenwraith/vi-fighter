...
var topics = []topicDef{
	...
	},
	{
		...
		Entries: []entryDef{
			{Actions: []string{"motion_left", "motion_down", "motion_up", "motion_right"}, Scope: input.SectionOverlay,
				Desc: "Select a card in TELEMETRY, or scroll a row in HELP; the arrow keys do the same"},
			...
			{Actions: []string{"overlay_filter"}, Scope: input.SectionOverlay,
				Desc: "Filter TELEMETRY cards by group or metric name; every space-separated term must match. Enter keeps the filter, Escape clears it"},
			{Actions: []string{"overlay_activate"}, Scope: input.SectionOverlay, All: true,
				Desc: "Pin or unpin the selected card; pins lead the overlay, and a new pin turns the HUD on"},
	...
	},
	{
		...
		Entries: []entryDef{
			{Keys: ":t  :telemetry", Desc: "Open every live metric group as a card; / filters, Space pins"},
			{Keys: ":t save", Desc: "Snapshot every metric with the view and session state to vif-snap-<time>.jsonl in the log directory; the status bar shows the path"},
			{Keys: ":hud [on|off]  :t hud", Desc: "Draw the pinned cards live over the game area; pinning a card turns it on"},
			{Keys: ":t unpin", Desc: "Clear every pinned card"},
		},
	},
	{
