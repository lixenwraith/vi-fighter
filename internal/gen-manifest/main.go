...
func main() {
	...
	generateFile("../system/profile_gen.go", systemProfileTemplate, defs)
...
func parseSystems(comp *ast.CompositeLit) []SystemDef {
	...
	for _, elt := range comp.Elts {
		...
		result = append(result, SystemDef{
			...
			Type:        strings.TrimPrefix(constructor, "New"),
			Domain:      extractName(lit.Elts[2]),
			Required:    extractStrings(lit.Elts[3]),
			Optional:    extractStrings(lit.Elts[4]),
		})
	}
	return result
}
...
func extractName(expr ast.Expr) string {
	switch v := expr.(type) {
	case *ast.Ident:
		return v.Name
	case *ast.SelectorExpr:
		prefix := extractName(v.X)
		if prefix == "" {
			return v.Sel.Name
		}
...
func extractStrings(expr ast.Expr) []string {
	...
	result := make([]string, 0, len(list.Elts))
	for _, entry := range list.Elts {
		result = append(result, extractString(entry))
	}
	return result
}
...
{{- range .Systems }}
		{
			...
			Build: func() engine.System { return system.{{ .Constructor }}(w) },
		},
{{- end }}
	}
}
...
var systemProfileTemplate = template.Must(template.New("system-profile").Parse(`// Code generated by gen-manifest; DO NOT EDIT.
...
// Domain returns the declared state boundary for {{ .Name }}.
func (*{{ .Type }}) Domain() engine.SystemDomain { return {{ .Domain }} }

// Dependencies returns the declared requirements for {{ .Name }}.
func (*{{ .Type }}) Dependencies() engine.SystemDependencies {
