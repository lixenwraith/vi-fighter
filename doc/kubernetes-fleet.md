...
| H12 | **next** | **Finish the occupied lifecycle gates.** First-join expiry and owned-Service cleanup passed. An allocator-created off-box join reached `occupied` then `vacant`; automatic empty-grace expiry, rejoin inside the grace, drain on Job deletion, and one-player capacity remain. The procedure is [Batch H12](../deploy/guest/README.md#batch-h12-occupied-lifecycle-gates). | Each of the four produces its specified transition and preserves the same run throughout the reconnect grace, with the refusal text and the exit reason quoted from the session's own JSONL. |
| H14 | next | **Complete the node-local log pipeline.** Batches A-F passed, including standalone fan-in, outage independence, exact retained replay, the pinned-revision LogWisp update, and byte preservation through the deployed allocator proxy. H12's lifecycle gates and Batch G's reconciliation remain. | Every remaining gate and the final acceptance in `kube-todo.md` passes without lowering Restricted admission or putting Kubernetes in the log data path. |
