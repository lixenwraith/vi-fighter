...
> superseded by [the fleet logging pivot](kube-todo.md). Batches A-D are deployed
> and passed: each live session writes its self-tagged JSONL through the
> tmpfs-backed PVC, and allocator RBAC cannot read pod logs. The public log
> endpoint remains 501 until Batches E-F finish. Batch E's standalone service is
> installed with a read-only tmpfs view and loopback-only listener. Its
> two-session fan-in and byte-preservation gate passed; outage/replay is next.
...
| Logs and metrics | Each Job writes `<session-id>.jsonl` through the Bound local PVC to the capped node tmpfs. The standalone LogWisp node service has a read-only view and loopback-only listener; two-session fan-in preserved 606 sampled records byte-for-byte without drops. Its outage/replay gate and the allocator proxy remain, so `/vif/api/logs` still returns 501. |

The allocator-to-Kubernetes path is implemented and verified through an off-box
join. The standalone LogWisp node edge is installed and its fan-in passed; its
outage/replay gate and the website/nginx/allocator stream edge remain.
...
| H14 | next | **Complete the node-local log pipeline.** Batches A-D passed. Batch E's standalone service passed installation, isolation, and two-session fan-in; its outage/replay gate and Batch F's allocator byte proxy remain. | Every remaining gate and the final acceptance in `kube-todo.md` passes without lowering Restricted admission or putting Kubernetes in the log data path. |
