...
| H11 | built, awaiting its live gate | **Verify the player's source address at the pod.** `externalTrafficPolicy: Local` plus `pf rdr` should preserve it. The address is now recorded: `App.noteJoinerReport` emits `peer admitted` under the `admit` sub with `remote` and `declared`, and LogWisp excludes that sub from the published stream. | A remote join names the off-box client in `/var/log/vif-fleet/<id>.jsonl` and nowhere on the SSE. If it names the node or gateway, the routing is corrected before relying on admission limits; otherwise the limiter is one budget for the whole fleet. |
