...
> loopback LogWisp service preserves the files' event bytes. Batch E's outage
> gate proved allocation and gameplay continue without LogWisp; its bounded
> restart replay processed 1,841 records with 86 client-queue drops while an
> exact retained sentinel replayed. Batch F's tested allocator byte proxy is the
> next live cutover, so the deployed `/vif/api/logs` still returns 501.
...
outage independence, and retained replay. The allocator stream edge is implemented
and tested but not yet deployed; nginx and website routing follow its live gate.
