...
| Trust | Links are plaintext and unauthenticated by decision. What the coordinator *does* check is identity: a joiner reports its protocol, simulation fingerprint, capture and journal schemas, and tick interval, and a peer that does not match the offer is refused before it takes a roster slot. The seed, scenario and corpus are not compared but adopted — `PeerIdentity.SessionFrom` takes the coordinator's, so a guest simulates the session's world and types the session's text rather than its own. |
