...
| `network.link_loss_pct` (int) | The share of probes that went unanswered. An unanswered probe is the only loss signal this protocol has — nothing acknowledges a correction and nothing repairs an epoch. The badge names it past `StatusNetLossWarnPct`; `:session` always prints it. |
